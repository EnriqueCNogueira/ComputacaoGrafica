...
// A bola bate na barra quando os dois corpos se sobrepoem
// em X (0.05 da bola + 0.05 da barra) e em Y (0.05 + 0.2)
function colideBarra(txBarra, tyBarra){
    return Math.abs(txBola - txBarra) < 0.1
        && Math.abs(tyBola - tyBarra) < 0.25;
}

function atualizaAnimacao(){
    txBola += txBola_offset;

    if(colideBarra(-0.9, tyBE) || colideBarra(0.9, tyBD))
...
function mouseMove(event){
    ...
    // Limitar em ±0.8 para a barra não sair da tela
    tyBE = Math.min(Math.max(webglY, -0.8), 0.8);
    tyBD = tyBE;
}
